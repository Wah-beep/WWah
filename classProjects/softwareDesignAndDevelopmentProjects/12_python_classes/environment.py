...
class Environment:
    ...
    def __init__(self, width=500, height=500):
        ...
        # a list of gold randomly distributed around the environment
        self.golds = []
    ...
    def add_random_gold(self):
        # Create and add a random gold item to the invironment.
        gold = Gold().make_random(self.width,self.height)
    ...
    def update(self):
        ...
        for r in self.robots:
            ...
            if self.in_pit(r):
                r.not_alive()
            # determine if the robot has found a gold pot
            if self.found_gold(r):
                r.found_gold()
    ...
    def found_gold(self,r):
        '''
        Check if the given robot has found any gold in the environment.
        Returns True if the robot has found gold, otherwise False.
        '''
        for gold in self.golds:
            deltaX = math.pow(gold.middle[0]-r.location[0],2)
            deltaY = math.pow(gold.middle[1]-r.location[1],2)
            if math.pow(deltaX+deltaY,0.5) <= r.radius+gold.size/2:
                return True
            
        # if not found any gold, then ...
        return False
