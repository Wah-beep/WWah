...
class RobotWorld:
    ...
    def generate_neighbor(self,solution):
        ...
        '''
        # Randon index to modify
        index = random.randint(0, len(neighbor) - 1)
        
        # Randon new move
        new_move = random.choice(MOVE_CHOICES)
        
        #Making sure the new move is different from current move
        while new_move == neighbor[index]:
            new_move = random.choice(MOVE_CHOICES)
        
        # Replace the old move with the new one
        neighbor[index] = new_move
        '''
		# modify in some way ...
        #Approach #2

        # Randomly decides how many moves to make
        num_changes = random.randint(1, min(3, len(solution)))
        
        # Change multiple random moves
    ...
    def cost(self,solution):
        ...
        for mov in solution:
            ...
            if status == "dead":
                # Assign high cost for dying, penalizing early deaths more
                cost = self.die_cost - move_count
                return cost
            
            if status=="gold":
                # Cost is based on the number of moves taken
                self.last_move = move_count
                cost = move_count
                return cost
        # If gold isn't found, then assign to high cost    
        self.last_move = move_count
        cost = self.die_cost
        return cost